    ...
    override fun run(client: NextcloudClient): RemoteOperationResult<List<RetentionLabelInfo>> {
        ...
        return try {
            ...
            if (status == HttpStatus.SC_OK) {
                val response = governanceJson.decodeFromString<GovernanceOcsResponse<List<RetentionLabelInfo>>>(
                    getMethod.getResponseBodyAsString()
                )
                val data = response.ocs.data
                RemoteOperationResult<List<RetentionLabelInfo>>(true, getMethod).apply { setResultData(data) }
            } else {
