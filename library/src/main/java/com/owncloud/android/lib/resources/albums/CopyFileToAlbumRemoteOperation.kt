    ...
    constructor(
        ...
        override fun run(client: OwnCloudClient): RemoteOperationResult<Any> {
            ...
            if (mTargetRemotePath == mSrcRemotePath) {
                ...
                val albumsUrl = "${client.baseUri}/remote.php/dav/photos/${client.userId}/albums"
                try {
                    copyMethod =
                        CopyMethod(
                            client.getFilesDavUri(this.mSrcRemotePath),
                            "$albumsUrl${WebdavUtils.encodePath(mTargetRemotePath)}",
                            false
                        )
                    val status =
                    ...
                    result =
                        when (status) {
                            HttpStatus.SC_MULTI_STATUS -> {
                                processPartialError(copyMethod)
                            }
         ...
         * @throws org.apache.jackrabbit.webdav.DavException If the status code is other than MultiStatus
         * or if obtaining the response XML document fails
         */
