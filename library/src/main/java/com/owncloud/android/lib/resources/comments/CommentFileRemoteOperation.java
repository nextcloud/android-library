...
public class CommentFileRemoteOperation extends RemoteOperation<Void> {
    ...
    public RemoteOperationResult<Void> run(NextcloudClient client) {
        ...
        try {
            ...
            requestBody.put(ACTOR_ID, client.getUserId());
            requestBody.put(ACTOR_TYPE, ACTOR_TYPE_VALUE);
            requestBody.put(VERB, VERB_VALUE);
            requestBody.put(MESSAGE, message);

            postMethod = new PostMethod(url, false, requestBody.get());
